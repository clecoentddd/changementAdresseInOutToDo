...
import { DB1SuccessfullyUpdatedEvent } from '../../stores/DB1Repository';
...
export class ToDoDB1Terminator {
  ...
  static async handleEvent(event) {
    try {
      ...
      const todo = await toDoChangeAddress.getTodosForDB1Update(event.trackingId);
      if (!todo) {
      ...
      await toDoChangeAddress.updateDB1Complete(event.trackingId);
